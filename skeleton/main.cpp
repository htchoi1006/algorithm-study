...
void render_scene()
{
  ...
  mat_view = g_cameras[g_cam_select_idx].get_view_matrix();
  if (gluPerspective){
    mat_proj = glm::perspective(glm::radians(camera.fovy()), 1.0f, 0.1f, 100.0f);
  }
  else{
...
void write(std::string fname){
  ...
  {
    glm::vec3 t=g_objects[i].get_translate();
    glm::vec3 s=g_objects[i].get_scale();


    fout<<"object ["<<i+1<<"]  "<<g_objects[i].get_name()<<std::endl;
  ...
  {
    glm::vec3 p=g_cameras[i].position();
    glm::vec3 f=g_cameras[i].front_direction();
    glm::vec3 u=g_cameras[i].up_direction();


    fout<<"camera ["<<i<<"]"<<std::endl;
